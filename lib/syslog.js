...
exports.decodeMessage = function(msg, callback) {
	try {
		...
		try {
			var i = msg.indexOf(">");
			var pri = parseInt(msg.substr(1, i - 1), 10);
			...
			try {
				...
				if (aheader.indexOf("[") !== -1 && aheader.indexOf("]") !== -1) {
					...
					syslog.msg = msg.slice(msg.indexOf(syslog.app + "[" + syslog.pid + "]:") + syslog.app.length + syslog.pid.length + 4);
				} else {
					syslog.app = s[4].substr(0, s[4].length - 1);
					syslog.msg = msg.slice(msg.indexOf(syslog.app + ":") + syslog.app.length + 2);
				}
